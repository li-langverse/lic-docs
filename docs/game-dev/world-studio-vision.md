# Li World Studio & Li Engine â master vision
...
**Li World Studio** is an **agent-native, provably correct** platform on **Li Engine**: one runtime for **games**, **simulation** (RL, automotive, robotics, additive manufacturing, scientific/engineering, **AI drug design**), with **killer UI/UX**, **publication-grade export**, and **Cursor SDK** agents â while **`lic build`** keeps proof first.
...
| Pillar | Meaning |
|--------|---------|
| **One engine** | Same `li-world` Â· `li-scene` Â· `li-physics-*` Â· `li-render` â no CARLA/GROMACS/slicer forks |
| **AI-first** | Local models default; agents via MCP + `@cursor/sdk` |
| **Killer UX** | 60 fps, adaptive layouts, â¤3 clicks for primary flows ([PH-UX](#17-ph-ux--killer-uiux)) |
| **Research + ship** | Deterministic replay, repro bundles, `li-player` for games |
| **Read like Python** | `def`, `import`, prose names â [philosophy.md](../language/philosophy.md) |
...
| Sim â  game | **Profiles** on one engine ([PH-SIM](#18-li-engine--runtime-profiles)) |
...
| Scene / UI | `li-scene`, `li-ui` | Stubs â studio chrome |
| Gameplay chem | `physics.chem` | Reactions stub â **not** QM |
...
â  li-studio â killer UX, studio.adaptive, studio.publish, command palette â
â  li-studio-ai + @cursor/sdk (PH-AGENT)                                    â
ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ¤
â  RUNTIME PROFILES: game | sim_rl | sim_automotive | sim_robotics |       â
â    sim_additive | sim_scientific | sim_drug_design                        â
ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ¤
â  Li Engine: li-world Â· li-scene Â· li-assets Â· li-physics-* Â· li-render   â
â             li-voxel (PH-VOXEL) Â· li-sim (step/replay/sensors)            â
ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ¤
â  Domain packs: automotive Â· robotics Â· additive Â· scientific Â· drug       â
â  li-chem (DFT/TDDFT/AI) Â· li-ml (async JobGraph)                          â
ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ¤
â  lic build (proof) Â· PH-PORT targets Â· PH-HW CUDA/HIP/LKIR                â
ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
```
...
One **`sim.step` / `game_step`** family â not duplicate physics.

| Profile | Use | Domain packs |
|---------|-----|----------------|
| `game` | Ship via `li-player` | â |
| `sim_rl` | Async env pools â engine tick | PH-ML |
...
## 6. PH-GD â World Studio editor (game authoring)
...
```text
User prompt â studio.ai / Cursor SDK â world.apply_patch
  â lic check (JSON diagnostics) â lic build â viewport / sim / export
```
...
Graphical CFD, MD (`md_lennard_jones`), heat, fluids, orbital â `li-physics-*` + `sim.viz`. Tier-2 benches run **in** the engine viewport.
...
Roche **Lab-in-the-Loop**âclass workflow: hypothesis â generate â **DFT/TDDFT** (`li-chem`) â lab ingest â retrain. **`studio.adaptive`** panels by stage/role.

RFC: [drug-design-lab-loop-rfc.md](specs/drug-design-lab-loop-rfc.md)

---

## 13. Quantum chemistry â `li-chem` (PH-QM)
...
Backends: native Li â GPU/LKIR â trusted ORCA/Psi4. RFC: [li-chem-qm-rfc.md](specs/li-chem-qm-rfc.md)
...
Four parallelism axes: sample (env pools), host prefetch, GPU streams, cluster (Triton-distributedâclass). Default `ml.runtime.mode = "async_parallel"`.

RFC: [ml-async-parallel-rfc.md](specs/ml-async-parallel-rfc.md)

---

## 17. PH-UX â killer UI/UX

| Target | Metric |
|--------|--------|
| Viewport | â¥60 fps |
| Panel switch | &lt;100 ms |
| AM export | â¤3 clicks |
...
## 19. PH-HW â `lig`, multi-vendor GPU, LKIR
...
| LKIR | Li Kernel IR under `packages/lig/lkir/` â CUDA/HIP/Metal/SPIR-V |

**Import:** `import lig` (replaces `import gpu` / package `li-gpu`).

**Portability:** `lic build --target <triple>` â RFC: [portable-targets-rfc.md](specs/portable-targets-rfc.md), [lig-rfc.md](specs/lig-rfc.md)

---

## 20. PH-COMPLY â critical packages
...
| Area | Target |
|------|--------|
| Game | Text â playable &lt;5 min (local 8B) |
| UX | SUS â¥75; 60 fps viewport |
| Sim | One `world.li` across game + sim profiles |
| AM | Sim pass â export 3MF/G-code |
